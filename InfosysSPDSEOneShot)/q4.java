...
public class q4 {
     ...
     * @param cost An n x c matrix where cost[i][j] is the cost to paint house i with color j
...
2. HOW IT WORKS STEP-BY-STEP:
   ...
   - House i (From House 1 to n - 1):
     If we paint house i with color j, the previous house (i - 1) CANNOT be color j.
...
3. SPECIAL CASE FOR 3 COLORS (c = 3):
   ...
   It simplifies directly to:
     dp[i][0] = cost[i][0] + Math.min(dp[i - 1][1], dp[i - 1][2]); // min of other 2
     dp[i][1] = cost[i][1] + Math.min(dp[i - 1][0], dp[i - 1][2]); // min of other 2
     dp[i][2] = cost[i][2] + Math.min(dp[i - 1][0], dp[i - 1][1]); // min of other 2
...
4. FINAL ANSWER & BUDGET CHECK:
   - The answer is the minimum value in the last row: min(dp[n - 1][0], ..., dp[n - 1][c - 1]).
